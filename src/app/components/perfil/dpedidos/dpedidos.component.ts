...
export class DpedidosComponent implements OnInit {
      ...
      response=>{
        ...
        if(response.data != undefined){
          ...
          response.detalles.forEach((element:any) => {
              this._guestService.obtener_review_producto_cliente(element.producto.id).subscribe(
                response=>{

                  let emitido = false;
                  response.data.forEach((element_:any) => {
                     if(element_.cliente == localStorage.getItem('id')){
                      emitido = true;
                     }
                  });
  ...
  openModal(item:any){
    this.review = {};
    this.review.producto = item.producto.id;
