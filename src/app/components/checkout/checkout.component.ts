...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    paypal.Buttons({
      ...
      onApprove : async (data:any,actions:any)=>{
        ...
        this.venta.direccion = this.direccion_envio.id;
        ...
        this._guestService.registro_compra_cliente(this.venta,this.token).subscribe(
          response=>{
            this.btn_load = false;
            this._router.navigate(['/cuenta/pedidos/'+response.data.id]);
          },
          error=>{
            console.log(error);
  ...
  obtener_direccion(){
    this._guestService.obtener_direccion_todos_cliente(localStorage.getItem('id'),this.token).subscribe(
      response=>{
  ...
  select_region(){
    ...
    this._guestService.get_Procincias().subscribe(
      response=>{
        response.forEach((element:any) => {
            if(element.departmentid == this.direccion.region){
  ...
  select_provincia(){
    ...
    this._guestService.get_Distritos().subscribe(
      response=>{
        response.forEach((element:any) => {
          if(element.provinceid == this.direccion.provincia){
            this.distritos.push(
              element
            );
          }
  ...
  obtener_carrito(){
    this._guestService.obtener_carrito_cliente(this.user_lc.id,this.token).subscribe(
      response=>{
      ...
      response=>{
        if(response.venta){
          ...
          let data = {
            ...
            items: items,
            back_urls: {
                failure: "http://localhost:5000/verificar-pago/failure/"+this.direccion_envio.id+'/'+this.venta.cupon+'/'+this.envio+'/'+this.tipo_descuento+'/'+this.valor_descuento+'/'+this.total_pagar+'/'+this.subtotal,
                pending: "http://localhost:5000/verificar-pago/pending/"+this.direccion_envio.id+'/'+this.venta.cupon+'/'+this.envio+'/'+this.tipo_descuento+'/'+this.valor_descuento+'/'+this.total_pagar+'/'+this.subtotal,
                success: "http://localhost:5000/verificar-pago/success/"+this.direccion_envio.id+'/'+this.venta.cupon+'/'+this.envio+'/'+this.tipo_descuento+'/'+this.valor_descuento+'/'+this.total_pagar+'/'+this.subtotal,
            },
    ...
    this._guestService.pedido_compra_cliente(this.venta,this.token).subscribe(
      response=>{
        ...
        if(response.venta != undefined){
          this.btn_load = false;
          this._router.navigate(['/cuenta/pedidos',response.venta.id]);
        }else{
          iziToast.show({
              title: 'ERROR',
              titleColor: '#FF0000',
